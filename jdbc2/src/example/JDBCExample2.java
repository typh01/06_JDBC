...
public class JDBCExample2 {
	public static void main(String[] args) {
		...
		try {
			 ...
			 * -> Connection을 생성하면 기본적으로 true(켜져 있음)
			 * -> 매번 수동으로 끔
			...
			pstmt.setString(2, pw); // pw 변수 값에 '' 추가하여 sql 대입
			pstmt.setString(3, name); // name 변수 값에 '' 추가하여 sql 대입
			
			/* SQL 수행 후 결과 반환 받기
			 * - SELECT : excuteQuery() - ResultSet 반환
			 * - DML(INSERT UPDATE DELETE) : executeUpdate() -> 행의 개수(int) 반환
			 *  */
			
			// DML 수행으로 조작된 행의 개수 반환됨
			int result = pstmt.executeUpdate();
			
			/* result 값에 따라서 SQL 성공/실패 여부 출력 + 실패 시 0
